...
def _get_address(business, addresses_id=None, address_type=None):
    ...
    if addresses_id:
        if business.is_legel_entity:
            ...
            rv = (
                db.session.query(AlternateName, Address)
                .filter(AlternateName.id == Address.alternate_name_id)
                .filter(AlternateName.identifier == business.identifier)
                .filter(Address.id == addresses_id)
                .one_or_none()
            )
        if rv:
